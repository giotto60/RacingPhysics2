...
export class SkidMarks {
  ...
  constructor(scene: THREE.Scene, private params: Params) {
    for (let i = 0; i < MAX_SEGMENTS; i += 1) {
      ...
      this.indices[t + 1] = v + 1;
      this.indices[t + 2] = v + 2;
      this.indices[t + 3] = v + 2;
      this.indices[t + 4] = v + 1;
      this.indices[t + 5] = v + 3;
    }
  ...
  update(wheels: WheelState[]): void {
    ...
    const halfWidth = this.params.suspension.wheelWidth * 0.5;
    ...
    wheels.forEach((wheel, i) => {
      ...
      if (!previous || previous.distanceToSquared(current) < 0.0025) return;
      ...
      // just past the limit leaves a faint grey line, a fully saturated one
      // leaves solid black.
      const t = clamp((slip - p.skidThreshold) / Math.max(0.05, 1.4 - p.skidThreshold), 0, 1);
      const alpha = t * p.skidOpacity;
      const shade = 0.22 - t * 0.16;
